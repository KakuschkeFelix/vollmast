...
import {FlagValue, VersionFlag, FlagScope} from "../../types";
import {PrismaModel} from "../../types/flag-prisma-client";
import {KeyGeneratorService} from "../key-generator.service";

export class VersionFlagService implements FlagHandler<VersionFlag> {

    constructor(public model: PrismaModel<VersionFlag>, private keyGenerator: KeyGeneratorService) {}

    async setFlag(key: string, config: Omit<VersionFlag, 'key'>, scope: FlagScope): Promise<VersionFlag> {
        try {
    ...
    async getFlag(key: string, scope: FlagScope): Promise<VersionFlag> {
        try {
            const completeKey = this.keyGenerator.generateKey(key, scope);
    ...
    async getFlagValue(key: string, scope: FlagScope): Promise<FlagValue<VersionFlag>> {
        try {
            const completeKey = this.keyGenerator.generateKey(key, scope);
            return this.model.findUnique({
                where: {
